...
import { canRead, canWrite, stringNullOrEmpty } from '../lib';
...
export abstract class Bucket<ConfigType extends BucketConfigOptions = any, NativeResponseType = any> implements IBucket<ConfigType, NativeResponseType> {

    public readonly name: string;
    public readonly absoluteName: string;
    public readonly config: ConfigType;
    public readonly provider: IStorageProvider<ConfigType, NativeResponseType, Bucket<ConfigType, NativeResponseType>>;
    public nativeProperites: any = {};

    constructor(provider: IStorageProvider, alias: string, config: ConfigType | string) {
        this.provider = provider;
        this.absoluteName = alias;
    ...
    /**
     * Parse the url string to options
     * @param url the config url
     */
    public abstract parseConfig(config: string | ConfigType): ConfigType;

    public validateConfig(): void {
        if (stringNullOrEmpty(this.config.name)) {
            throwError(`Invalid bucket config! The name must be provided!`);
        }
    }
