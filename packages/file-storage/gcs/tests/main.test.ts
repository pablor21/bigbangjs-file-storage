...
describe('GCS tests', () => {
    ...
    test('Storage operations', async () => {
        ...
        const provider01: GCSProvider = (await storage.addProvider<GCSProvider>('provider01', urlProviderConfig02)).result;
        ...
        await provider01.listUnregisteredBuckets();
        ...
        // check native path
        expect(bucket01.getNativePath(file01)).toBe('gs://' + bucket01.gcsBucket.name + '/file-01.txt');
        ...
        expect(file04.getNativePath()).toBe('gs://' + bucket02.gcsBucket.name + '/' + file04.getAbsolutePath());
